...
result <- LMMsetup(score ~ Machine, dat = Machines, ref = list("Worker", c("Worker", "Machine")))
dim(result$Z)
colnames(result$Z)
...
LMMprof <- function(theta, setup) {
  ...
  # Skip iteration if matrix is not positive definite
  if (any(eigenvalues <= 0)) {
    warning("Non-positive definite matrix encountered with current theta. Returning Inf.")
    return(Inf)  # Returning Inf as a penalty to indicate skipping
  }
